...
Stm32::Stm32() : dcbSerialParams({0}), timeouts({0})
{
        ...
        {
                logger.log(LogLevel::ERROR_, "Open post fail [%s]", __FUNCTION__);
                post_stast = ERROR_;
        }
        else
        {
                logger.log(LogLevel::INFO, "Open post success");
        }

        if (!GetCommState(Post, &dcbSerialParams))
        {
                logger.log(LogLevel::ERROR_, "Get dcbSerialParams fail in %s", __FUNCTION__);
                post_stast = ERROR_;
                CloseHandle(Post);
        }
